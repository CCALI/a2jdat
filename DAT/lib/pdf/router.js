...
const storageRouter = new Router().use(user.middleware).get('/:comboId', async (req, res) => {
  ...
  storage.saveTemplatePdf(username, guideId, templateId, filepath).then(() => unlink(filepath)).then(() => res.json({ ok: true })).catch(error => errorRes(res, error));
