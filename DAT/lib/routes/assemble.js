...
async function assemble(req, res) {
  ...
  if (isSingleTemplateAssemble) {
    return createPdfForTextTemplates(htmlOptions, pdfOptions).then(pdfStream => {
...
async function getTemplatesForGuide(username, guideId, fileDataUrl) {
  ...
  const templatesPromises = templateIds.map(templateId => paths.getTemplatePath({ guideId, templateId, username, fileDataUrl }).then(path => files.readJSON({ path })));

  const isActive = template => template.active === 'true' || template.active === true;

  return Promise.all(templatesPromises).then(templates => templates.filter(isActive));
}
