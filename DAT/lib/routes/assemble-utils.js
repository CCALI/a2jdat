...
function uniq(list) {
  return list.reduce((list, item) => list.includes(item) ? list : [...list, item], []);
}

function mergeGuideVariableWithAnswers(variables, answers) {
  const allUniqueKeys = uniq([...Object.keys(variables), ...Object.keys(answers)]);
...
const isPdfTemplate = template => template.rootNode && template.rootNode.tag === 'a2j-pdf';
...
function filterTemplatesByCondition(answers) {
  return function (template) {
    ...
    const shouldBeIncluded = evalAuthorCondition(Object.assign(state, { answers }));
